...
def zipdir():
    ...
    FILENAME = (
        "./skins/Customs-"
        + request.json["racer"].lower()
        + "-"
        + str(request.json["number"])
    )
...
def process_img():
    if request.method == "POST":
        ...
        response = requests.post(
            "http://api.resmush.it/ws.php?img=" + IMAGES_FOLDER + image.filename,
            headers={"Content-Type": "application/json"},
        )
    return jsonify(
        {"msg": "Image reduced", "filename": json.loads(response.text)["dest"]}
    )
